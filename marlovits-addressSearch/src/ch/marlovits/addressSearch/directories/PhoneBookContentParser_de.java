...
public class PhoneBookContentParser_de extends PhoneBookContentParser {
	...
	String[] extractionSpecs =
		{
			...
			PhoneBookEntry.FLD_PROFESSION + ":TITLE:0", PhoneBookEntry.FLD_NOTE + ":NOTE:0"
		};
	...
	public HashMap<String, String> extractKontaktFromList(){
		...
		// und Name/firstname ist auch nicht getrennt -> ergo einfach direkt alles aus der Homepage
		// ziehen
		// das geht schneller...
	...
	public HashMap<String, String> extractMaxInfo(HashMap<String, String> kontaktHashMap){
		...
		/*
		 * many many times, this vCard link doesn't work (not even on the homepage itself...) and
		 * returns an empty doc usually the vCard link on the detail page is working -> try to read
		 * this one NO - CHANGE: ALWAYS read vCard from detailPage since we need to read some
		 * additional info from the detail page anyway
		 */
		...
		for (int i = 0; i < extractionSpecs.length; i++) {
			...
			String vCardValue =
				formatString(vCardParser.getVCardValue(vCardKey, Integer.parseInt(infoIndex)));
		...
		result.put(PhoneBookEntry.FLD_PHONE1, formatPhoneNumber(result
			.get(PhoneBookEntry.FLD_PHONE1)));
		result.put(PhoneBookEntry.FLD_FAX, formatPhoneNumber(result.get(PhoneBookEntry.FLD_FAX)));
		result.put(PhoneBookEntry.FLD_PHONE2, formatPhoneNumber(result
			.get(PhoneBookEntry.FLD_PHONE2)));
		result.put(PhoneBookEntry.FLD_MOBILE, formatPhoneNumber(result
			.get(PhoneBookEntry.FLD_MOBILE)));
	...
	public URL getURL(String name, String geo, int pageNum){
		...
		urlPattern =
			"http://www2.dastelefonbuch.de/?la={0}&kw={1}&ci={2}&ciid=&cmd=search&cifav=0&mdest=sec1.www1&vert_ok=1&recfrom={3}&reccount={4}"; //$NON-NLS-1$
	...
	public boolean hasCitiesList(){
		...
		return true;
		/*
		 * if (moveTo("<div id=\"content\" class=\"hitlist place-sel\">")) { boolean hasNextCategory
		 * = moveTo("<div class=\"functionbar\">"); while(hasNextCategory) { String category =
		 * extractTo("</div>").trim(); // if last char is ")" then it IS a category if
		 * (category.endsWith(")")) { return true; } hasNextCategory =
		 * moveTo("<div class=\"functionbar\">"); } } return false;
		 */
	}
	...
	public String[][] getCitySuggestions(final String part){
		String result =
			readContent("http://www.telefonbuch.de/Suggestor?source=sm&max_res=20&where=" + part,
				"UTF-8", this.htmlReadTimeout);
		result =
			result.replaceAll("zeige\\(0, new Array\\(\"ort\"\\), new Array\\(new Array\\(\"", "");
	...
	/*
	 * http://www.telefonbuch.de/Suggestor?source=sm&max_res=20&where=Singen zeige(0, new
	 * Array("ort"), new Array(new Array("Singen Hohentwiel"), new Array("Singhofen"), new
	 * Array("Singwitz Gem. Obergurig"), new Array("Singlis Stadt Borken (Hessen)"), new
	 * Array("Singelbert Gem. Reichshof"), new Array("Singen Gem. Remchingen"), new
	 * Array("Singer Gem. Sankt Wolfgang"), new Array("Singenbach"), new Array("Singern"), new
	 * Array("Singlding Stadt Erding")));
	 */
}
