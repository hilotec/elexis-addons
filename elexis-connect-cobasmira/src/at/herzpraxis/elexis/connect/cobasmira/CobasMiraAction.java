...
public class CobasMiraAction extends Action implements ComPortListener {
	...
	public void run() {
		if (isChecked()) {
			if (running) {
				_ctrl.sendBreak();
				_ctrl.close();
				setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin("at.herzpraxis.elexis.connect.cobasmira", "icons/cobasred.ico"));
				running=false;
				System.out.println("Cobas finished");
				return;
			} 
			setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin("at.herzpraxis.elexis.connect.cobasmira", "icons/cobasgreen.ico"));
			running=true;
			...
			if (msg == null) {
				String timeoutStr = Hub.localCfg.get(Preferences.TIMEOUT,
						Messages.getString("CobasMiraAction.DefaultTimeout")); //$NON-NLS-1$
				int timeout = 20;
				try {
				...
				SWTHelper.showError(Messages
						.getString("CobasMiraAction.RS232.Error.Title"), //$NON-NLS-1$
						msg);
			}
		} else {
			if (_ctrl.isOpen()) {
	...
	private void initConnection() {
		...
		_ctrl = new CobasMiraConnection(
				Messages.getString("CobasMiraAction.ConnectionName"), //$NON-NLS-1$
				Hub.localCfg.get(Preferences.PORT, Messages
						.getString("CobasMiraAction.DefaultPort")), Hub.localCfg.get( //$NON-NLS-1$
								Preferences.PARAMS,
								Messages
										.getString("CobasMiraAction.DefaultParams")), //$NON-NLS-1$
				this);
		...
		if (Hub.localCfg.get(Preferences.LOG, "n").equalsIgnoreCase("y")) { //$NON-NLS-1$ //$NON-NLS-2$
			try {
				...
				SWTHelper.showError(Messages
						.getString("CobasMiraAction.LogError.Title"), //$NON-NLS-1$
		...
		background = Hub.localCfg.get(Preferences.BACKGROUND, "n")
				.equalsIgnoreCase("y");

	}

	/**
	 * Fetch lines coming from the serial connection starting with 20. These are incoming result values.
	 * Generate a Probe object, where the initialization of the Probe object takes care about the proper
	 * parsing of the input.
	 * If the received probe adheres to certain criteria pass it on to further processing.
	 * 
	 * @param conn, data
	 */
	public void gotData(AbstractConnection conn, final byte[] data) {
		...
		for (int i = 0; i < strArray.length; i++) {
			if (strArray[i].startsWith("20")) { // ^20 Denotes a result line
				...
				if (Hub.getSystemLogLevel()>4) {
					_rs232log.log(Messages.getString("CobasMiraAction.ProbeInput")+": "+probe.getIdent()+" "+probe.getTestKuerzel()+" "+probe.getResult());
				}
				...
				if (!probe.getIdent().equalsIgnoreCase(
					...
					_rs232log.log(Messages.getString("CobasMiraAction.ProbeInputFault")+":"+strArray[i]); //$NON-NLS-1$
				}
			}
		...
		ElexisEventDispatcher.reload(LabItem.class); // Sync the modifications on PersistentObjects - especially Value, i.e. a LabItem in this case
	}

	/**
	 * Process a valid probe. Query the database for a Patient according to the ident of the probe,
	 * present a MessageWindow with a Timeout and subsequently insert the code into the DB calling the
	 * write method of probe.
	 * 
		...
		Desk.getDisplay().asyncExec(new Runnable() {	
			public void run() {
				...
				if (patientList.size() == 1) {
					...
					String text = MessageFormat.format(Messages
							.getString("CobasMiraAction.ValueInfoMsg"),
							patientDeviceStr, patientElexisStr, probe.getTestKuerzel(), probe.getDate().toString(TimeTool.DATE_GER),probe.getResult());
					if (background==false) {		
					...
					showError("ERROR", Messages.getString("CobasMiraAction.WrongPatientID")+": "+probe.getIdent());	
				}		
			}
		});
	 ...
	 * 
	 * @see
	 * ch.elexis.rs232.AbstractConnection.ComPortListener#gotBreak(ch.elexis
	...
	public void gotBreak(AbstractConnection conn) {
		...
		_rs232log.logEnd();
		SWTHelper.showError(Messages
				.getString("CobasMiraAction.RS232.Break.Title"), Messages //$NON-NLS-1$
	...
	public void timeout() {
		...
		SWTHelper.showError(Messages
				.getString("CobasMiraAction.RS232.Timeout.Title"), //$NON-NLS-1$
