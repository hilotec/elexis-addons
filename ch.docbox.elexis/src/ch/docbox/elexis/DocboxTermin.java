...
public class DocboxTermin {
	...
	public boolean create(AppointmentType appointment, String bereich){
		...
		try {
			...
			if (elexisTermin == null) {
				elexisTermin =
					new Termin(getTerminUniqueId(appointment.getId(),bereich, true), bereich, tag, von, bis, terminType,
						terminStatus);
			}
			if (twoday) {
				if (elexisTerminDayAfter == null) {
					elexisTerminDayAfter =
						new Termin(getTerminUniqueId(appointment.getId(),bereich,false), bereich, tag, von, bis, terminType,
							terminStatus);
				}
			}
			setLocked(false);
			...
			if (!twoday) {
				...
				elexisTerminDayAfter.set(new String[] {
					"BeiWem", "Tag", "Beginn", "Typ", "Status", "ErstelltVon", "Dauer"
				}, new String[] {
					bereich, ttUntil.toString(TimeTool.DATE_COMPACT), Integer.toString(0),
			...
			setGrund(grund);
			setLocked(true);
			log.log("Termin " + elexisTermin.getId()+" "+elexisTermin.getText()+" "+elexisTermin.getGrund()+" - "+elexisTermin.getDay(),Log.DEBUGMSG);
	...
	public boolean delete(){
		...
		if (elexisTermin != null && elexisTermin.exists()) {
			log.log("Terminid " + elexisTermin.getId()+" "+elexisTermin.getText()+" "+elexisTermin.getGrund()+"- "+elexisTermin.getBereich(),Log.DEBUGMSG);
