 ...
 */
public class DocboxDocumentsView extends ViewPart implements ISaveablePart2,
		IActivationListener, HeartListener {
	...
	private final ElexisEventListenerImpl patientSelectionListener = new ElexisEventListenerImpl(
	...
	private String[] getColumnLabels() {
		String columnLabels[] = { Messages.DocboxDocumentsView_DateSent,
				Messages.DocboxDocumentsView_Title,
				Messages.DocboxDocumentsView_Sender,
				Messages.DocboxDocumentsView_Patient, 
	...
	private Font createBoldFont(Font baseFont) {
		...
		Font font = new Font(baseFont.getDevice(), fd.getName(),
	...
	public void createPartControl(Composite parent) {
		...
		actionOpenAttachments = new Action(
				Messages.DocboxDocumentsView_Action_AttachmentsOpen) {
			public void run() {
				ISelection selection = tableViewer.getSelection();
				Object obj = ((IStructuredSelection) selection)
						.getFirstElement();
		...
		actionShowCdaDocument = new Action(
				Messages.DocboxDocumentsView_Action_ShowCdaDocument) {
			public void run() {
				ISelection selection = tableViewer.getSelection();
				Object obj = ((IStructuredSelection) selection)
						.getFirstElement();
				...
				final Clipboard cb = new Clipboard(Desk.getDisplay());
				cb.setContents(new Object[] { cdaMessage.getCda() },
						new Transfer[] { textTransfer });
			}
		};

		actionDeleteDocument = new Action(
				Messages.DocboxDocumentsView_Action_Delete) {
			public void run() {
				ISelection selection = tableViewer.getSelection();
				Object obj = ((IStructuredSelection) selection)
						.getFirstElement();
				if (obj != null) {
					CdaMessage cdaMessage = (CdaMessage) obj;
					MessageBox messageBox = new MessageBox(Desk.getDisplay()
							.getActiveShell(), SWT.ICON_WARNING | SWT.OK
							| SWT.CANCEL);
					messageBox
							.setText(Messages.DocboxDocumentsView_Action_Delete);
					messageBox
							.setMessage(String
									.format(Messages.DocboxDocumentsView_Action_DeleteConfirmMsg,
											cdaMessage.getTitle()));
		...
		actionCreatePatient = new Action(
				Messages.DocboxDocumentsView_Action_CreatePatient) {
			public void run() {
				ISelection selection = tableViewer.getSelection();
				Object obj = ((IStructuredSelection) selection)
						.getFirstElement();
				...
				if (obj != null) {
					...
					if (name!=null) {
						...
						ElexisEventDispatcher
								.fireSelectionEvent((PersistentObject) p);
							...
							if (object instanceof CdaMessage) {
								...
								if (cda != null) {
									...
									String firstName = cdaChXPath
											.getPatientFirstName();
									String geburtsdatum = cdaChXPath
											.getPatientDateOfBirth();
									if (cdaChXPath.getPatientNumber() != null) {
										String patId = new Query<Patient>(
												Patient.class)
												.findSingle(
														"PatientNr", "=", cdaChXPath.getPatientNumber());//$NON-NLS-1$ //$NON-NLS-2$
										if (patId != null) {

											Patient elexisPatient = Patient
													.load(patId);
											if (elexisPatient != null
													&& elexisPatient.getName()
															.equals(lastName)
													&& elexisPatient
															.getVorname()
															.equals(firstName)) {
												log.log("selecting patient by id with "
														+ lastName
														+ ", "
														+ firstName,
														Log.DEBUGMSG);
												ElexisEventDispatcher
														.fireSelectionEvent(elexisPatient);
												return;
											}
										}
									}
									if (KontaktMatcher.findPerson(lastName,
											firstName, geburtsdatum, null,
											null, null, null, null,
											CreateMode.FAIL) != null) {
										log.log("selecting patient by demographics "
												+ lastName + ", " + firstName,
												Log.DEBUGMSG);
										Patient elexisPatient = KontaktMatcher
	...
	public void visible(boolean mode) {
		if (mode == true) {
			...
			ElexisEventDispatcher.getInstance().removeListeners(reloadListener,updateListener,patientSelectionListener);
		}
