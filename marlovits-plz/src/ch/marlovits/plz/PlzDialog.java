...
public class PlzDialog extends TitleAreaDialog {
	...
	/**
	 * Setzen der internen Variablen lang für die Internationalization (DE/FR/IT/EN, etc) Falls
	 * irgendetwas schief geht, dann Default = "DE"
	 */
	...
	protected Control createDialogArea(Composite parent){
		/*
		 * Layout: - Composite top mit 2 Spalten, füllt ganzen Dialog enthält immer
		 * Bezeichnungen/Labels - rechtes Panel von Composite top enthält: - einfaches Feld ODER -
		 * rightComposite mit 2 Spalten
		 * 
		 * __top____________________________________________ || || ___rightComposite______________
		 * || || || | | | || || || ------------------------------- || || || || || ||
	...
	private void addKantonsFields(){
		...
		try {
			for (int i = 0; i < labelKantonArray.length; i++) {
				if (labelKantonArray[i] != null)
					labelKantonArray[i].dispose();
				if (comboIsoKantonArray[i] != null)
					comboIsoKantonArray[i].dispose();
				if (comboNameKantonArray[i] != null)
					comboNameKantonArray[i].dispose();
				if (compKantonArray[i] != null)
		...
		for (int kantonIndex = 0; kantonIndex < numOfRegions; kantonIndex++) {
			...
			String[] tmpKantonsListeSubCode =
				getKantonsListe("kantonsubcode", landIso2, kantonIndex, "kantonsubcode", lang);
			String[] tmpKantonsListeName =
				getKantonsListe("kantonname", landIso2, kantonIndex, "kantonsubcode", lang);
	...
	/**
	 * Synchronisieren der Kürzels-Combo mit dem Namens-Combo. Auswahl Kürzel -> Setzen Namen
	 * 
	...
	/**
	 * Synchronisieren der Kürzels-Combo mit dem Namens-Combo. Auswahl Name -> Setzen Kürzel
	 * 
	 * @author Harald Marlovits
	 * 
	 */
	...
	protected void okPressed(){
		...
		String kantonauswaehlen =
			getLandFieldValue(landIso2Field.getText(), "kantonauswaehlen", lang);
		if ((Integer.parseInt(kantonauswaehlen) != 0) && (cbKantonIso.getText() == null)) {
			if (focusField == null)
				focusField = cbKantonIso;
		...
		if (act == null) {
			...
			act =
				new Plz(cbLandCombo.getText(), landIso2Field.getText(), plzField.getText(), ort
					.getText(), strasse.getText(), cbKantonName.getText(), cbKantonIso.getText());
		} else {
	...
	private int getNumOfRegions(final String landIso, final String locale){
		...
		String sql =
			"select count(*) as regionCount from (select kantonindex from ch_marlovits_kanton where upper(kantonlanguage) = '"
				+ locale.toUpperCase()
				+ "' and upper(kantonland) = '"
				+ landIso.toUpperCase()
				+ "' and deleted = '0' group by kantonindex) as foo";
	...
	private String getKantonIndexName(final String landIso, final int kantonIndex,
		final String locale){
		String sql = "";
	 ...
	 * 
	 * @param landIso2
	 *            : suche Eintrag mit dieser LandISO2
	 * @param feldName
	 *            : lese Wert für dieses Feld
	...
	private String getLandFieldValue(final String landIso2, final String feldName, String locale){
		...
		String res = null;
		ResultSet rs =
			stm.query("select " + feldName + " from land where upper(landiso2) = '"
				+ landIso2.toUpperCase() + "' and landlanguage = '" + locale.toUpperCase() + "'");
		try {
			rs.next();
	...
	public String[] getLaenderListe(final String fieldName, final String locale){
		...
		int numOfRows = 0;
		ResultSet rs =
			stm.query("select count(*) as cnt from CH_MARLOVITS_LAND where upper(landlanguage) = '"
		...
		rs =
			stm.query("select " + fieldName
				+ " from CH_MARLOVITS_LAND where upper(landlanguage) = '" + locale.toUpperCase()
				+ "' " + isoQuery + " order by landsorting, landname");
		try {
			int i = 0;
	...
	public String[] getKantonsListe(final String fieldName, final String landIso2,
		final int kantonIndex, final String orderBy, final String locale){
		// Datenbank anzapfen
		Stm stm = j.getStatement();
		...
		ResultSet rs =
			stm
				.query("select count(*) as cnt from CH_MARLOVITS_KANTON where upper(kantonlanguage) = '"
					+ locale.toUpperCase()
					+ "' and kantonland = "
					+ "'"
					+ landIso2
					+ "' and kantonindex=" + kantonIndex);
		try {
			rs.next();
			numOfRows = rs.getInt("cnt");
		} catch (SQLException e1) {
		...
		rs =
			stm.query("select " + fieldName
				+ " from CH_MARLOVITS_KANTON where upper(kantonlanguage) = '"
				+ locale.toUpperCase() + "' and kantonland = " + "'" + landIso2
				+ "' and kantonindex = " + kantonIndex + " order by " + orderBy);
		if (rs == null) {
			j.releaseStatement(stm);
			return null;
		}
	 ...
	 * 
	 * @param fieldControl
	 *            : Feld, das getestet werden soll
	 * @param fieldName
	 *            : Name für das Feld, das in der Fehlermeldung angezeigt werden soll
