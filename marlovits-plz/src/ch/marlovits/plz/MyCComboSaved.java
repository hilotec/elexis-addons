 ...
 * list and issues notificiation when an item is selected from the list.
 * <p>
...
public final class MyCComboSaved extends Composite {
	...
	public MyCComboSaved(Composite parent, int style){
		...
		/*
		 * // meine eigene Popup-Liste, mehrere Spalten // die Shell, in welcher die Liste erstellt
		 * wird popup2 = new Shell (getShell (), SWT.NO_TRIM);
		 * 
		 * TableViewer dropTable = new TableViewer(popup2, SWT.BORDER | SWT.SINGLE |
		 * SWT.FULL_SELECTION); GridData gd = new GridData(); gd.horizontalAlignment = SWT.FILL;
		 * gd.verticalAlignment = SWT.FILL; gd.grabExcessHorizontalSpace = true;
		 * gd.grabExcessVerticalSpace = true;
		 * 
		 * dropTable.getControl().setLayoutData(gd); Table myTable2 = dropTable.getTable(); myTable2
		 * .setHeaderVisible(false); myTable2 .setLinesVisible(false); // Die einzelnen Einträge
		 * abfragen und in einen String-Array und dann in die Liste schreiben //rs =
		 * stm.query("select ort27 from " + PlzEintrag.getTableName2() + " where " + landClause +
		 * " and lower(ort27) like lower(" + JdbcLink.wrap(currText + "%") +
		 * ")  and plztyp != 80 order by ort27"); Stm stm =
		 * PersistentObject.getConnection().getStatement(); String shownFields = "ort27, plz, land";
		 * String landClause = " lower(land) = lower(" + JdbcLink.wrap("CH") + ") "; ResultSet rs =
		 * stm.query("select " + shownFields + " from " + PlzEintrag.getTableName2() + " where " +
		 * landClause + " and lower(ort27) like lower(" + JdbcLink.wrap("D" + "%") +
		 * ")  and plztyp != 80 order by ort27"); for (int iiii = 0; iiii < 3; iiii++) { TableColumn
		 * col1 = new TableColumn(myTable2, SWT.NULL); col1.setText("Column " + iiii); col1.pack();
		 * } try { while (rs.next()) { TableItem tableItem1 = new TableItem(myTable2, SWT.NULL);
		 * tableItem1.setText(new String[] {rs.getString("ort27"), rs.getString("plz"),
		 * rs.getString("land")}); } } catch (SQLException e1) { } myTable2.setTopIndex(40);
		 */
	 ...
	 * Deselects an item.
	 * <p>
	 * If the item at an index is selected, it is deselected. If the item at an index is not
	 * selected, it remains deselected. Indices that are out of range are ignored. Indexing is zero
	 * based.
	 * 
	 * @param index
	 ...
	 *                (ERROR_WIDGET_DISPOSED) when the widget has been disposed
	 * @exception SWTError
	 *                (ERROR_ERROR_CANNOT_GET_ITEM_HEIGHT) when the operation fails
	 */
	 ...
	 * <p>
	 * The list is searched starting at 0 until an item is found that is equal to the search item.
	 * If no item is found, -1 is returned. Indexing is zero based.
	 * 
	 * @param string
	 *            the search item
	 ...
	 * Gets the index of an item.
	 * <p>
	 * The widget is searched starting at start including the end position until an item is found
	 * that is equal to the search itenm. If no item is found, -1 is returned. Indexing is zero
	 * based.
	 * 
	 * @param string
	 *            the search item
	...
	void listEvent(Event event){
		...
		case SWT.FocusOut: {
			...
			event.display.asyncExec(new Runnable() {
				public void run(){
					if (MyCComboSaved.this.isDisposed())
						return;
	 ...
	 * Indexing is zero based.
	 * 
	 * This operation will fail when the index is out of range or an item could not be removed from
	 * the OS.
	 * 
	 * @param index
	 *            the index of the item
	 ...
	 * <p>
	 * Indexing is zero based. The range of items is from the start index up to and including the
	 * end index.
	 * 
	 * This operation will fail when the index is out of range or an item could not be removed from
	 * the OS.
	 ...
	 * Selects an item.
	 * <p>
	 * If the item at an index is not selected, it is selected. Indices that are out of range are
	 * ignored. Indexing is zero based.
	 * 
	 * @param index
	 *            the index of the item
	 * 
	 ...
	 * 
	 * This operation will fail when the index is out of range or an item could not be changed in
	 * the OS.
	 * 
	 * @param index
	 *            the index for the item
	...
	/*
	 * public boolean isPrintableChar(char c) { Character.UnicodeBlock block =
	 * Character.UnicodeBlock.of( c ); return (!Character.isISOControl(c)) && c !=
	 * KeyEvent.CHAR_UNDEFINED && block != null && block != Character.UnicodeBlock.SPECIALS; }
	 */
}
