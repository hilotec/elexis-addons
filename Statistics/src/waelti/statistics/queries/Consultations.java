...
public class Consultations extends AbstractTimeSeries {
	...
	private boolean inPeriod(String date) {
		...
		return (fallDate.compareTo(this.getStartDate()) >= 0 && fallDate
	...
	private void increment_patient_counter(Patient patient, int year) {
		...
		this.getList().get(year - this.firstCohort)[index] = ((Integer) this
				.getList().get(year - this.firstCohort)[index]) + 1;
	}

	private void add_male(int birthYear, Konsultation cons) {
		this.getList().get(birthYear - this.firstCohort)[7] = (Integer) this
				.getList().get(birthYear - this.firstCohort)[7]
				+ ((Double) cons.getUmsatz()).intValue();

		this.getList().get(birthYear - this.firstCohort)[8] = (Integer) this
				.getList().get(birthYear - this.firstCohort)[8] + 1;
	}

	private void add_female(int birthYear, Konsultation cons) {
		this.getList().get(birthYear - this.firstCohort)[2] = (Integer) this
				.getList().get(birthYear - this.firstCohort)[2]
				+ ((Double) cons.getUmsatz()).intValue();

		this.getList().get(birthYear - this.firstCohort)[3] = (Integer) this
				.getList().get(birthYear - this.firstCohort)[3] + 1;
	}
	...
	private boolean validate_data(Patient patient) {
		...
		boolean gender = patient.getGeschlecht().equals("m")
				|| patient.getGeschlecht().equals("w");
	...
	public void setCohortSize(String cohortSize) throws SetDataException {
		...
		try {
			...
			throw new SetDataException(
					"Kohortengrösse muss eine positive Ganzzahl sein.");
		}
