...
public abstract class AbstractTimeSeries extends AbstractQuery {
	...
	protected void initData() {
		...
		this.getStartDate().set(getStartDate().get(Calendar.YEAR),
				Calendar.JANUARY, 1);

		this.setEndDate(Calendar.getInstance());
		this.getEndDate().set(this.getEndDate().get(Calendar.YEAR),
	...
	public void metaSetEndDate(String endDate) throws SetDataException {
		...
		if (cal.compareTo(this.getStartDate()) < 0) {
			throw new SetDataException(
					"Enddatum vor Anfangsdatum. Bitte ändern Sie das Start- "
