...
public class QueryInputDialog extends TitleAreaDialog {
	...
	private void fillQueryTable() {
		...
		for (IConfigurationElement element : elements) {
			try {
				AbstractQuery query = (AbstractQuery) element
	...
	private void initDescription(Composite parent) {
		...
		this.description.setLayoutData(SWTHelper.getFillGridData(1, true, 1,
				true));
	}
	...
	/**
	 * When "ok" is pressed, the input data will be read, a query object created
	 * and set in the given action. If there is any exception setting any of the
	 * data given by the input window, a message will be displayed and nothing
	 * happens.
	 */
	@Override
	...
	/**
	 * Returns true if the query was created. False if any exception occured.
	 * The exception message will be shown.
	 */
