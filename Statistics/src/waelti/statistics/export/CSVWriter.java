...
public class CSVWriter {
	...
	public static File write(AbstractQuery query, String file)
			throws IOException {
		return CSVWriter.write(query, new File(file));
	}

	/**
	 * Writes the data to a csv file with ';' as a delimiter and returns the
	 * file. If writing fails, null will be returned.
	 * 
	 * @throws IOException
	 */
	...
	public static File writer(ResultMatrix matrix, File file)
			throws IOException {
	...
	private static void writeHeading(FileWriter writer, List<String> headings)
			throws IOException {
		Object[] objects = headings.toArray();
		CSVWriter.writeRow(writer, objects);
	}

	private static void writeRow(FileWriter writer, Object[] objects)
			throws IOException {
