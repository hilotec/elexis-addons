...
public class RefreshQueryAction extends NewQueryAction {
	...
	public RefreshQueryAction(OutputView outputView) {
		...
		this.setToolTipText("Aktualisiert die aktuelle Auswertung mit "
				+ "den neuen Daten.");
		this.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(
				"Waelti.Statistics", "icons/database_refresh.png"));
	}

	@Override
	protected void getInput() {
		// TODO: remove this hot fix as soon as gw changed backgroundjobListener
		this.getView().getQuery().removeListener(
				this.getView().getNewQueryAction());
		try {
			this
					.setConfiguredQuery(this.getView().getQueryOptions()
							.getQuery());
