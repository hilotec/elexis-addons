 ...
 * </dl>
 */
enum MARLOVITSCOMBO_DISPLAYLINES {
 ...
 * interface.
	...
	public Point computeSize(int wHint, int hHint, boolean changed){
		...
		for (int i = 0; i < table.getItemCount(); i++) {
			...
			textWidth =
				Math.max(gc.stringExtent(items[i].getText(textLinkedColumnIndex)).x, textWidth);
		}
		gc.dispose();
	...
	/*
	 * void createLists(int numOfLists) { // *** adjusting style
	 * *************************************************** int style = getStyle (); int tableStyle =
	 * SWT.SINGLE; if ((style & SWT.FLAT) != 0) tableStyle |= SWT.FLAT; if ((style &
	 * SWT.RIGHT_TO_LEFT) != 0) tableStyle |= SWT.RIGHT_TO_LEFT; if ((style & SWT.LEFT_TO_RIGHT) !=
	 * 0) tableStyle |= SWT.LEFT_TO_RIGHT;
	 * 
	 * // *** create lists == columns ******************************************* // disposing of
	 * unused lists/columns int currListCount = 0; if (lists != null) { currListCount =
	 * lists.length; if (numOfLists < currListCount) { for (int i = numOfLists; i < currListCount;
	 * i++) { lists[i].dispose(); } } } List[] newLists = new List[numOfLists]; if (lists != null) {
	 * System.arraycopy(lists, 0, newLists, 0, lists.length); } lists = newLists; // creating
	 * lists/columns for (int listIx = 0; listIx < numOfLists; listIx++) { // detect or create
	 * column List currList = null; if (listIx < currListCount){ currList = lists[listIx]; } else {
	 * currList = new List(popup, tableStyle); lists[listIx] = currList; } // set font/colors, etc
	 * if (font != null) currList.setFont(font); if (foreground != null) currList.setForeground
	 * (foreground); if (background != null) currList.setBackground (background); // set listeners
	 * for lists int [] listEvents = {SWT.MouseUp, SWT.MouseDown, SWT.Selection, SWT.Traverse,
	 * SWT.KeyDown, SWT.KeyUp, SWT.FocusIn, SWT.Dispose, SWT.MouseHover, SWT.MouseMove}; for (int
	 * eventIx = 0; eventIx < listEvents.length; eventIx++) {
	...
	void createPopup(String[][] items, int selectionIndex){
		...
		if (items != null) {
			...
			for (int colIx = 0; colIx < numOfColumns; colIx++) {
				table.getColumn(colIx).setWidth(1000); // bug workaround for Windows XP which makes
														// colums smaller but not bigger in pack()
		...
		int[] tableEvents =
			{
				SWT.MouseMove, SWT.MouseUp, SWT.Selection, SWT.Traverse, SWT.KeyDown, SWT.KeyUp,
				SWT.FocusIn, SWT.FocusOut, SWT.MouseMove, SWT.DRAG, SWT.NONE, SWT.MouseDown,
				SWT.MouseExit, SWT.MouseEnter
			};
	...
	void dropDown(boolean drop){
		...
		if (tableSize.y > verticalSpace) {
			if (System.getProperties().getProperty("os.name").equals("Windows XP")) {
				tableSize.y =
					(int) ((verticalSpace / table.getItemHeight()) - 2) * table.getItemHeight()
						+ headerHeight;
			} else {
				tableSize.y =
					(int) (verticalSpace / table.getItemHeight()) * table.getItemHeight()
						+ headerHeight;
			}
		}
		...
		popup.setBounds(rect.x, rect.y + comboSize.y, tableSize.x + 2 - sbWidth + leftMarginOffset,
			tableSize.y + 2);
 ...
 * <code>Combo.LIMIT.
	...
	void tableEvent(Event event){
		...
		case SWT.MouseMove:
			...
			if (isTrackingTable == false) {
				...
				// windows behaviour
				Point pt =
					new Point((int) (MouseInfo.getPointerInfo().getLocation().getX()),
				...
				bounds =
					new Rectangle(bounds.x, bounds.y, bounds.width
						- ((table.getVerticalBar() != null) ? table.getVerticalBar().getSize().x
							- leftMarginOffset : 0), bounds.height);
					...
					if (globalPt.y > (bounds.y + bounds.height)) {
						...
						if (globalPt.y < bounds.y) {
							...
							if (table.getSelectionIndex() != startTrackingItem) {
								...
								text.setText(table.getItem(startTrackingItem).getText(
									textLinkedColumnIndex));
								text.selectAll();
							}
							drawFocus(newSelection);
						}
		...
		if ((event.type != SWT.MouseMove) && (event.type != SWT.MouseDown)
			&& (event.type != SWT.MouseUp) && (event.type != SWT.Selection)) {
	 ...
	 */
	/*
	 * //+++++ Data public void remove(String string) { checkWidget(); if (string == null)
	 * SWT.error(SWT.ERROR_NULL_ARGUMENT); int firstFound = indexOf(string); if (firstFound < 0)
	 * return; for (int i = 0; i < lists.length; i++) { lists[i].remove(firstFound); } }
	 */
	/*
	 * //+++++ Data public void remove(int columnIx, String string) { checkWidget(); if (string ==
	 * null) SWT.error(SWT.ERROR_NULL_ARGUMENT); int firstFound = indexOf(columnIx, string); if
	 * (firstFound < 0) return; for (int i = 0; i < lists.length; i++) {
 ...
 * <code>add'ing the new item at that index.
	 ...
	 */
	/*
	 * //+++++ Data public void setItems(int columnIx, String[] items) { checkWidget(); if
	 * ((columnIx < 0) || (columnIx >= table.getItemCount())) { return; }
	 * lists[columnIx].setItems(items); if (!text.getEditable()) text.setText(""); //$NON-NLS-1$ }
	 */
	// IO +++++
	public void setItems(String[][] items){
		...
		/*
		 * checkWidget (); int numOfLists = items.length; createLists(numOfLists); for (int i = 0; i
		 * < numOfLists; i++) { String[] itm = items[i]; lists[i].setItems(itm); } if
		 * (!text.getEditable()) text.setText(""); //$NON-NLS-1$
		 */
	}
 ...
 * display incorrectly.
	...
	void drawFocus(int item){
		...
		table.redraw(origFocus.x - leftMarginOffset, pt.y + vOffset + 1, origFocus.width
			- ((table.getVerticalBar() != null) ? table.getVerticalBar().getSize().x
				- leftMarginOffset : 0), itemHeight, true);
		// *** draw new focus ***************************************************
		if (item != currSel) {
			vOffset = itemInList * itemHeight;
			origFocus = table.getBounds();
			origFocus = new Rectangle(0, 0, origFocus.width, origFocus.height - origFocus.y);
			listGC.drawFocus(origFocus.x - leftMarginOffset, pt.y + vOffset + 1, origFocus.width
				- ((table.getVerticalBar() != null) ? table.getVerticalBar().getSize().x
					- leftMarginOffset : 0), itemHeight);
		}
