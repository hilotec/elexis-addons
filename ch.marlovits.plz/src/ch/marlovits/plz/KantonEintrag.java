...
public class KantonEintrag extends PersistentObject {
	...
	static {
		...
		addMapping(TABLENAME, "Kantonname=kantonname", "Kantonfullcode=kantonfullcode",
			"Kantonsubcode=kantonsubcode", "Kantonland=kantonland", "Kantonindex=kantonindex",
			"Kantonkind=kantonkind", "Kantonwikilink=kantonwikilink",
			"Kantonlanguage=kantonlanguage", "Deleted=deleted", "lastupdate=lastupdate");
		
		// Erstellen der Tabelle in der Datenbank
		createOrModifyTable(createDB);
		/*
		 * KantonEintrag version = KantonEintrag.load("1"); if (!version.exists()) {
		 * createOrModifyTable(createDB); } else { VersionInfo vi = new
		 * VersionInfo(version.getKantonName()); if (vi.isOlder(VERSION)) { // Update-Script für
	...
	public KantonEintrag(String kantonname, String kantonfullcode, String kantonsubcode,
		String kantonland, String kantonindex, String kantonkind, String kantonwikilink,
		String kantonlanguage){
		create(null);
		set(new String[] {
			"Kantonname", "Kantonfullcode", "Kantonsubcode", "Kantonland", "Kantonindex",
			"Kantonkind", "Kantonwikilink", "Kantonlanguage"
		}, new String[] {
			kantonname, kantonfullcode, kantonsubcode, kantonland, kantonindex, kantonkind,
			kantonwikilink, kantonlanguage
		});
	}
