...
public class MCCombo extends Composite {
	...
	public MCCombo(Composite parent, int style){
		...
		int[] comboEvents = {
			SWT.Dispose, SWT.Move, SWT.Resize, SWT.DRAG, SWT.DragDetect, SWT.Traverse
		};
		for (int i = 0; i < comboEvents.length; i++)
			this.addListener(comboEvents[i], listener);
		
		int[] popupEvents = {
			SWT.Close, SWT.Paint, SWT.Deactivate, SWT.MouseMove, SWT.MouseExit
		};
		...
		int[] listEvents =
			{
				SWT.MouseUp, SWT.Selection, SWT.Traverse, SWT.KeyDown, SWT.KeyUp, SWT.FocusIn,
				SWT.FocusOut, SWT.MouseMove, SWT.DRAG, SWT.NONE, SWT.MouseDown, SWT.MouseHover,
				SWT.MouseExit, SWT.MouseEnter
			};
		for (int i = 0; i < listEvents.length; i++)
			table.addListener(listEvents[i], listener);
		
		// int [] tableViewerEvents = {SWT.MouseUp, SWT.Selection, SWT.Traverse, SWT.KeyDown,
		// SWT.KeyUp, SWT.FocusIn, SWT.FocusOut, SWT.MouseMove, SWT.DRAG, SWT.NONE, SWT.MouseDown,
		// SWT.MouseHover, SWT.MouseExit};
		// for (int i=0; i<tableViewerEvents.length; i++)
		// table.getColumn(0).addListener(tableViewerEvents [i], listener);
		
		initAccessible();
	}
	
	/*
	 * void columnEvent(Event event) { switch(event.type) { case SWT.MouseUp:
	 * System.out.println("MouseUp"); break; case SWT.Selection: System.out.println("Selection");
	 * break; case SWT.Traverse: System.out.println("KeyUp"); break; //case SWT.KeyDown: // break;
	 * case SWT.KeyUp: System.out.println("KeyUp"); break; case SWT.FocusIn:
	 * System.out.println("FocusIn"); break; case SWT.FocusOut: System.out.println("FocusOut");
	 * break; case SWT.MouseMove: System.out.println("MouseMove"); break; case SWT.DRAG:
	 * System.out.println("DRAG"); break; case SWT.NONE: System.out.println("NONE"); break; case
	 * SWT.MouseDown: System.out.println("MouseDown"); break; case SWT.MouseHover:
	 * System.out.println("MouseHover"); break; case SWT.MouseExit: System.out.println("MouseExit");
	 * break; default: System.out.println("other"); //textEvent(event); break; } }
	 */
	 ...
	 * SWT.error(SWT.ERROR_NULL_ARGUMENT); list.add(string); }
	 */
	// TODO
	/*
	 * public void add(String string, int index) { checkWidget(); if (string == null)
	 * SWT.error(SWT.ERROR_NULL_ARGUMENT); list.add(string, index); }
	 */
	...
	// TODO
	/*
	 * public String[] getItems() { checkWidget(); return list.getItems(); }
	 */
	...
	// TODO
	/*
	 * public int indexOf(String string) { checkWidget(); if (string == null)
	 * SWT.error(SWT.ERROR_NULL_ARGUMENT); return list.indexOf(string); }
	 */
	// TODO
	/*
	 * public int indexOf(String string, int start) { checkWidget(); if (string == null)
	 * SWT.error(SWT.ERROR_NULL_ARGUMENT); return list.indexOf(string, start); }
	 */
	void initAccessible(){
	...
	/**
	 * Sets the fields linked to this control. The data found in the current selection will be
	 * inserted into these fields. The fields are defined in returnFields[]. Every object that has
	 * the setText method can be used in this array.
	 * 
	...
	boolean setLinkedFields(){
		...
		for (int fieldIx = 0; fieldIx < returnFields.length; fieldIx++) {
			try {
				...
				String[] input = {
					new String(tableItem.getText(fieldIx))
				};
				// call the method
	...
	// TODO
	/*
	 * public void remove(String string) { checkWidget(); if (string == null)
	 * SWT.error(SWT.ERROR_NULL_ARGUMENT); if (isOldStyle){ list.remove(string); } else {
	 * //table.remove(string); } }
	 */
	...
	// TODO
	/*
	 * public void setItem(int index, String string) { checkWidget(); if (string == null)
	 * SWT.error(SWT.ERROR_NULL_ARGUMENT); list.setItem(index, string); }
	 */
	...
	void textEvent(Event event){
		...
		case SWT.KeyDown: {
			...
			if (event.keyCode == SWT.PAGE_DOWN) {
				...
				table.setSelection(Math.min(table.getItemCount() - 1, table.getSelectionIndex()
					+ numOfDisplayedRows - 1));
				text.setText(table.getSelection()[0].getText());
				text.setSelection(text.getText().length());
				event.doit = false;
				handled = true;
			...
			if (event.keyCode == SWT.ARROW_UP || event.keyCode == SWT.ARROW_DOWN) {
				...
				if (event.keyCode == SWT.ARROW_UP) {
					switch (oldIndex) {
					case -1: {
						// no item is currently selected -> save Text in theText, select last item,
						// set text field, select end of text
						theText = text.getText();
						table.setSelection(table.getItemCount() - 1);
						text.setText(table.getSelection()[0].getText());
						text.setSelection(text.getText().length());
					...
					case 0: {
						// first item is currently selected -> deselect, set saved text, select end
						// of text
						table.deselectAll();
						text.setText(theText);
						text.setSelection(text.getText().length());
					...
					default: {
						// another item is selected -> select previous item, set text field, select
						// end of text
				...
				if (event.keyCode == SWT.ARROW_DOWN) {
					switch (oldIndex) {
					case -1: {
						// no item is currently selected -> save Text in theText, select first item,
						// set text field, select end of text
						theText = text.getText();
						table.setSelection(0);
						text.setText(table.getSelection()[0].getText());
					...
					default: {
						if (oldIndex == table.getItemCount() - 1) {
							// last item is currently selected -> deselect, set saved text, select
							// end of text
							table.deselectAll();
							text.setText(theText);
							text.setSelection(text.getText().length());
						} else {
							// another item is selected -> select next item, set text field, select
							// end of text
							table.setSelection(oldIndex + 1);
							table.setSelection(oldIndex + 1);
							text.setText(table.getSelection()[0].getText());
