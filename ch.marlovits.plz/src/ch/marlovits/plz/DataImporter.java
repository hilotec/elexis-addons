...
enum NameTypes {
	COUNTRYNAME, CURRENCYNAME, CITYNAME, CURRENCYNAME_OTHER
}

enum CountryCodesInputFields {
	iso2, iso3, isoNum, fips, name, capital, area, population, continent, tld, currencyCode,
		currencyName, phone, postalCodeFormat, postalCodeRegex, languages, geonameId, neigbours,
		equivalentfips
}

enum NameCodesInputFields {
	id, nameId, language, name, isPreferredName, isShortName
}

enum PlzInputFields {
	onrp, plzTyp, plz, zusatzZiffer, ort18, ort27, kanton, sprachCode, sprachCode2, sortierFile,
		briefZustellung, gemeindeNr, gueltigAb
}

public class DataImporter {
	// *** Konstanten für die Iso-Land-Daten
	private static final String COUNTRYCODES_URL =
		"http://download.geonames.org/export/dump/countryInfo.txt";
	private static final String COUNTRYCODES_FIELDDELIMITER = "\t";
	
	private static final String NAMES_SOURCE =
		"D:\\Dokumente und Einstellungen\\Harry\\Desktop\\alternateNames.txt";
	...
	static public void importCountryData(final String lang){
		try {
			...
			// grösstes lastupdate einlesen - alle Einträge, die <= lastUpdate sind, werden später
			// als
			// deleted markiert, da alle noch vorhandnen anderen aktualisiert sind mit neuerem
			// Zeitmarker
			StringBuffer sql = new StringBuffer(300);
			sql.append("select lastupdate from " + CountryEintrag.getTableName1()
				...
				if (!firstChar.equals("#")) { // skip comment lines
					...
					country
						.updateEntry(
							...
							(fields.length - 1 < CountryCodesInputFields.neigbours.ordinal()) ? null
									: fields[CountryCodesInputFields.neigbours.ordinal()],
							currLanguage);
					// übersetzte Landesbezeichnung in ch_marlovits_name erstellen oder
					// aktualisieren
					nameEintrag = new NameEintrag(countryID);
					nameEintrag.updateEntry(fields[CountryCodesInputFields.geonameId.ordinal()],
						currLanguage, fields[CountryCodesInputFields.name.ordinal()], "", "",
						Integer.toString(NameTypes.COUNTRYNAME.ordinal()));
					// übersetzte Währungsbezeichnung in ch_marlovits_name erstellen oder
					// aktualisieren
					/*
					 * // ich benutze die Werte aus http://unicode.org nameEintrag = new
					 * NameEintrag(countryID + "_" + NameTypes.CURRENCYNAME.ordinal());
					 * nameEintrag.updateEntry(fields[CountryCodesInputFields.geonameId.ordinal()],
					 * currLanguage, fields[CountryCodesInputFields.currencyName.ordinal()], "", "",
					...
					for (int i = 0; i < languageArray.length; i++) {
						...
						countryID =
							fields[CountryCodesInputFields.iso2.ordinal()] + "_" + currLanguage;
						try {
							// in geoNames nach dem übersetzten Namen suchen:
							// CountryCode=<xx> -> getCountryName()
							searchCriteria.setLanguage(currLanguage);
							searchCriteria.setCountryCode(fields[CountryCodesInputFields.iso2
								.ordinal()]);
							...
							nameEintrag.updateEntry(fields[CountryCodesInputFields.geonameId
								.ordinal()], currLanguage, countryName, "", "", Integer
								.toString(NameTypes.COUNTRYNAME.ordinal()));
			...
			for (int i = 0; i < languageArray.length; i++) {
				currLanguage = languageArray[i];
				// für die aktuelle Sprache alle Währungsbezeichnungen in ch_marlovits_names
				// einlesen
				String[][] languagesArray = getLanguagesTable(currLanguage);
				String langId;
				for (int langIx = 0; langIx < languagesArray.length; langIx++) {
					langId =
						languagesArray[langIx][0] + "_" + currLanguage + "_"
							+ NameTypes.CURRENCYNAME.ordinal();
					nameEintrag = new NameEintrag(langId);
					nameEintrag.updateEntry(languagesArray[langIx][0], currLanguage,
						languagesArray[langIx][1], "", "", Integer.toString(NameTypes.CURRENCYNAME
							.ordinal()));
					langId =
						languagesArray[langIx][0] + "_" + currLanguage + "_"
							+ NameTypes.CURRENCYNAME_OTHER.ordinal();
					nameEintrag = new NameEintrag(langId);
					nameEintrag.updateEntry(languagesArray[langIx][0], currLanguage,
						languagesArray[langIx][2], "", "", Integer
							.toString(NameTypes.CURRENCYNAME_OTHER.ordinal()));
				}
			}
			...
			sql.append("UPDATE CH_MARLOVITS_COUNTRY set deleted = " + JdbcLink.wrap("1")
				+ "where lastupdate <= " + lastUpdate + " and (entrylanguage != "
				+ JdbcLink.wrap("en") + " and entrylanguage != " + JdbcLink.wrap(lang) + ")");
	...
	static public void importNameData(){
		...
		try {
			...
			while ((fieldsLine = br.readLine()) != null) {
				...
				sql.setLength(0);
				sql.append("select count(*) as cnt from CH_MARLOVITS_NAME_GEONAMES where id = "
					+ JdbcLink.wrap(fieldsArray[NameCodesInputFields.id.ordinal()]) + ")");
				int count = PersistentObject.getConnection().queryInt(sql.toString());
				if (count == 0) {
					// Direktaufruf von SLQ, da Statement unten schnell Fehler ergibt bei riesiger
					// Datei
					// gibt wie unten stehender Code irgendwann einen Heap out of mem error, aber
					// vieeeeel später
					sql.setLength(0);
					sql
						.append("insert into CH_MARLOVITS_NAME_GEONAMES ( id, nameid, \"language\", name, ispreferredname, isshortname, deleted, lastupdate) values (");
					sql
						.append(JdbcLink.wrap(fieldsArray[NameCodesInputFields.id.ordinal()])
							+ ", ");
					...
					sql.append(JdbcLink.wrap(fieldsArray[NameCodesInputFields.language.ordinal()])
						+ ", ");
					sql.append(JdbcLink.wrap((fieldsArray.length - 1 < NameCodesInputFields.name
						.ordinal()) ? null : fieldsArray[NameCodesInputFields.name.ordinal()])
						+ ", ");
					sql.append(JdbcLink
						.wrap((fieldsArray.length - 1 < NameCodesInputFields.isPreferredName
							.ordinal()) ? null : fieldsArray[NameCodesInputFields.isPreferredName
							.ordinal()])
						+ ", ");
					sql
						.append(JdbcLink
							.wrap((fieldsArray.length - 1 < NameCodesInputFields.isShortName
								.ordinal()) ? null : fieldsArray[NameCodesInputFields.isShortName
								.ordinal()])
							+ ", ");
					...
					j.exec(sql.toString());
					/*
					 * der folgende Code geht nicht - gibt einen Heap out of memory error, ist nicht
					 * wegzukriegen... nameEintrag = new
					 * NameEintrag(fieldsArray[NameCodesInputFields.id. ordinal()],
					 * fieldsArray[NameCodesInputFields.nameId. ordinal()],
					 * fieldsArray[NameCodesInputFields.language. ordinal()], (fieldsArray.length -
					 * 1 < NameCodesInputFields.name.ordinal()) ? null :
					 * fieldsArray[NameCodesInputFields.name.ordinal()], (fieldsArray.length - 1 <
					 * NameCodesInputFields.isPreferredName.ordinal()) ? null :
					 * fieldsArray[NameCodesInputFields.isPreferredName.ordinal()],
					 * (fieldsArray.length - 1 < NameCodesInputFields.isShortName.ordinal()) ? null
					 * : fieldsArray[NameCodesInputFields.isShortName.ordinal()]);
					 */
				}
	...
	public static void geoNamesTest(){
		...
		/*
		 * ToponymSearchCriteria searchCriteria; searchCriteria = new ToponymSearchCriteria();
		 * searchCriteria.setQ("zurich"); ToponymSearchResult searchResult = null; try {
		 * searchResult = WebService.search(searchCriteria); } catch (Exception e) { // TODO
		 * Auto-generated catch block e.printStackTrace(); } for (Toponym toponym :
		 * searchResult.getToponyms()) { System.out.println(toponym.getName()+" "+
	...
	public static void importTabDelimited(){
		try {
			String fileName =
				"D:\\Dokumente und Einstellungen\\Harry\\Desktop\\DatenFürElexis\\plzs\\CH_Post\\plz_p1_20090325.txt";
			...
			while ((line = br.readLine()) != null) {
				...
				new PlzEintrag(fieldsArray[PlzInputFields.onrp.ordinal()],
					fieldsArray[PlzInputFields.plzTyp.ordinal()], fieldsArray[PlzInputFields.plz
						.ordinal()], fieldsArray[PlzInputFields.zusatzZiffer.ordinal()],
					fieldsArray[PlzInputFields.ort18.ordinal()], fieldsArray[PlzInputFields.ort27
						.ordinal()], fieldsArray[PlzInputFields.kanton.ordinal()],
					...
					fieldsArray[PlzInputFields.gueltigAb.ordinal()], "de", "", "ch");
			}
			return;
		} catch (Exception ex) {
			ExHandler.handle(ex);
					...
					if (currNode2.getNodeName().equals("currencies")) {
						...
						for (int k = 0; k < currencyNodeList.getLength(); k++) {
							...
							if (currNode3.getNodeName().equals("currency")) {
								...
								// Währungsbezeichnungen extrahieren: ohne attributes = Einzahl,
								// count="other": Mehrzahl
								String singular = "";
								String plural = "";
								for (int l = 0; l < currencySubNodeList.getLength(); l++) {
									Node currNode4 = currencySubNodeList.item(l);
									Node tmpNode;
									if ((currNode4.getNodeType() != 3)
										&& (currNode4.getNodeName() == "displayName")) {
										NamedNodeMap nnm = currNode4.getAttributes();
										String countVal =
											((tmpNode = nnm.getNamedItem("count")) == null) ? ""
													: tmpNode.getNodeValue();
										String altVal =
											((tmpNode = nnm.getNamedItem("alt")) == null) ? ""
													: tmpNode.getNodeValue();
										String draftVal =
											((tmpNode = nnm.getNamedItem("draft")) == null) ? ""
													: tmpNode.getNodeValue();
										if ((countVal.equals("")) && (altVal.equals(""))
											&& (draftVal.equals(""))) {
											singular = currNode4.getTextContent();
										}
										if ((countVal.equals("other")) && (altVal.equals(""))
											&& (draftVal.equals(""))) {
